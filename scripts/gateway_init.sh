...
# 3. 内核路由避让 (强制绕过 TUN，防止回环)
# --------------------------------------
apply_bypass_rules() {
    # 自动获取局域网段 (例如 10.10.1.0/24)
    local lan_cidr=$(ip -o -4 addr show dev "$IFACE" | awk '{print $4}' | head -n 1)
    
    # 定义需要强制直连的目标 (局域网段、常用国内 DNS)
    # 223.5.5.5 (Ali), 119.29.29.29 (Tencent), 114.114.114.114, 180.76.76.76 (Baidu)
    local bypass_targets=("$lan_cidr" "223.5.5.5/32" "119.29.29.29/32" "114.114.114.114/32" "180.76.76.76/32")
    
    # 补充用户定义的 LOCAL_CIDR
    if [ -n "$LOCAL_CIDR" ]; then
        bypass_targets+=("$LOCAL_CIDR")
    fi

    log "🛡️  正在配置内核路由避让 (优先级: 100)..."
    for target in "${bypass_targets[@]}"; do
        if [ -n "$target" ] && [[ "$target" == *"/"* ]]; then
            # 检查是否已存在，避免重复添加
            if ! ip rule show to "$target" | grep -q "lookup main"; then
                ip rule add to "$target" lookup main prio 100
            fi
        fi
    done
}
